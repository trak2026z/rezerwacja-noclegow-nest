...
echo "=== 11. Guest LIKE room (expect 200/201) ==="
...
echo "=== 13. Guest DISLIKE (switch from like) (expect 200/201) ==="
...
echo "=== 15. Guest reserves room (expect 200/201) ==="
...
echo "=== 18. Delete room as owner (200/204) ==="
DELETE_OWNER_CODE=$(curl -sS -o /dev/null -w "%{http_code}" -X DELETE "${API}/rooms/${ROOM_ID}" \
  -H "Authorization: Bearer ${OWNER_TOKEN}")
echo "HTTP $DELETE_OWNER_CODE"
if [[ "$DELETE_OWNER_CODE" == "200" || "$DELETE_OWNER_CODE" == "204" ]]; then
  echo "✅ Delete as owner: OK"; DELETE_OWNER_OK=true
else
